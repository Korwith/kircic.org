...
class MacDots {
    ...
    constructor(parent) {
        ...
        parent.appendChild(this.element);
        let buttons = ['red', 'yellow', 'green', 'settings'];
...
const CurrentLink = new SidebarButton(ExternalPagePane.element, { name: 'kircic.org' }, { icon: '../icon/stack.svg' });
CurrentLink.element.classList.add('active');
const SnapshotLink = new SidebarButton(ExternalPagePane.element, { name: 'snap.red', link: 'https://snap.red' }, { icon: '../icon/image.svg', size: '95%' });
new SidebarBreak(Sidebar.element);
const InternalPagePane = new SidebarInnerPane(Sidebar.element);
new TextSubheader(InternalPagePane.element, 'Navigation');
const HomeLink = new SidebarButton(InternalPagePane.element, { name: 'Home', link: '#home' }, { icon: '../icon/home.svg', size: '80%' });
const ProjectsLink = new SidebarButton(InternalPagePane.element, { name: 'Projects', link: '#projects' }, { icon: '../icon/cube.svg', size: '90%' });
const CodeLink = new SidebarButton(InternalPagePane.element, { name: 'Codebase', link: '#code' }, { icon: '../icon/code.svg', size: '92%' });
const SearchLink = new SidebarButton(InternalPagePane.element, { name: 'Search', link: '#search' }, { icon: '../icon/search.svg', size: '90%' });
new SidebarBreak(Sidebar.element);
