...
class CodebasePage extends Page {
    ...
    async initialize() {
        ...
        if (!data)
            return this.pushCodebaseNotification('error', 'There was an error fetching the GitHub API. Are you connected to the internet?');
        ...
        const root_button = new EntryElementFolder(this, root_listing, this.sidebar);
    ...
    async openEntry(path) {
        ...
        let current_entry = this.cache[path[0]];
        if (!current_entry) {
        ...
        for (let i = 1; i < path.length; i++) {
            ...
            if (current_entry.type === 'dir' && !current_entry.children) {
                ...
                if (!children_array)
                    throw new Error(`Failed to load directory: ${sub_path.join('/')}`);
                current_entry.children = {};
                for (const child of children_array)
                    current_entry.children[child.name] = child;
            }
        ...
        if (current_entry.type === 'dir' && !current_entry.children) {
            ...
            if (!children_array)
                throw new Error(`Failed to load directory: ${path.join('/')}`);
            current_entry.children = {};
            for (const child of children_array)
                current_entry.children[child.name] = child;
        }
    ...
    pushCodebaseNotification(type, error) {
...
class FileExplorer extends GlassPageSegment {
    codebase;
    constructor(codebase) {
        super(codebase);
...
class CodebaseMain extends FileExplorer {
    ...
    reflectEntry(entry) {
        ...
        if (entry.listing.type !== 'dir') {
            const class_name = FileIcons.getClassWithColor(entry.listing.name);
            const add_classes = class_name != null ? ['has_icon', ...class_name.split(' ')] : ['none'];
