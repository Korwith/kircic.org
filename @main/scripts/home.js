...
class SocialRow {
    element;
    constructor(parent, social_array) {
        this.element = new Pane(parent, { width: '100%', height: '40px' }).element;
        this.element.classList.add('social_holder');
        let names = Object.keys(social_array);
        for (var i = 0; i < names.length; i++) {
            let service_name = names[i];
            let service_data = social_array[service_name];
...
class Clock {
    ...
    #updateInfo() {
        ...
        let weekday = day_array[date.getDay()];
