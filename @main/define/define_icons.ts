interface IconEntry {
    icon: string;
    size?: string;
}

interface SocialEntry {
    gradient: GradientEntry;
    image: IconEntry;
    link: string;
    type: string;
}

interface SocialEntryList {
    [key: string]: SocialEntry;
}

interface GradientEntry {
    direction: string;
    colors: string[];
}

const Language_Icons: IconEntry[] = [
    { icon: '@main/icon/javascript.svg', size: '75%' },
    { icon: '@main/icon/typescript.svg', size: '75%' },
    { icon: '@main/icon/java.svg', size: '75%' },
    { icon: '@main/icon/lua.svg', size: '70%' },
    { icon: '@main/icon/html.svg', size: '80%' },
    { icon: '@main/icon/css.svg', size: '80%' },
    { icon: '@main/icon/linux.ico', size: '76%' },
    { icon: '@main/icon/certification1.png', size: '88%' },
    { icon: '@main/icon/certification2.png', size: '88%' },
    { icon: '@main/icon/certification3.png', size: '88%' },
    { icon: '@main/icon/certification4.png', size: '88%' },
];

const OS_Icons: IconEntry[] = [
    { icon: '@main/icon/linux.ico' },
    { icon: '@main/icon/debian.svg' },
    { icon: '@main/icon/ubuntu.svg' },
    { icon: '@main/icon/kubuntu.svg' },
    { icon: '@main/icon/opensuse.svg' },
    { icon: '@main/icon/fedora.svg' },
];

const Social_Icons: SocialEntryList = {
    'cashapp': {
        gradient: { direction: 'to bottom right', colors: ['#0cdb62', '#046b23'] },
        image: { icon: 'https://www.kircic.org/@main/icon/cashapp.svg', size: '58%' },
        link: 'https://cash.app/$thadreal',
        type: 'social',
    },
    'discord': {
        gradient: { direction: 'to bottom right', colors: ['#5975da', '#030d2f'] },
        image: { icon: 'https://www.kircic.org/@main/icon/discord.svg', size: '85%' },
        link: 'https://discord.com/invite/p8ZZXZqnag',
        type: 'social',
    },
    'github': {
        gradient: { direction: 'to bottom right', colors: ['#15171a', '#2b2e31'] },
        image: { icon: 'https://www.kircic.org/@main/icon/github.svg', size: '70%' },
        link: 'https://github.com/Korwith',
        type: 'social',
    },
    'instagram': {
        gradient: { direction: 'to bottom left', colors: ['#7455A2', '#EC287C', '#F07B2B', '#F1C65C'] },
        image: { icon: 'https://www.kircic.org/@main/icon/instagram.svg', size: '73%' },
        link: 'https://www.instagram.com/thadcoolfr/',
        type: 'social',
    },
    'youtube': {
        gradient: { direction: 'to top left', colors: ['#ff2222', '#9c0000'] },
        image: { icon: 'https://www.kircic.org/@main/icon/youtube.svg', size: '70%' },
        link: 'https://www.youtube.com/channel/UCLcCNfyLG_jQev4MdkHtCZw',
        type: 'social',
    },
    'monkeytype': {
        gradient: { direction: 'to bottom right', colors: ['#414141', '#272727'] },
        image: { icon: 'https://www.kircic.org/@main/icon/monkeytype.svg', size: '80%' },
        link: 'https://monkeytype.com/profile/kircic',
        type: 'social',
    },
};
